...
* 默认覆盖率门禁（make test-coverage，包含 PostgreSQL 与全部项目 Go 包）提升到至少 75%，关键安全与事务模块优先达到更高覆盖率。
...
* [x] 注销存储失败时接口返回错误，旧会话不会被当作已撤销。
* [x] 同一验证令牌并发消费时只有一个请求成功。
* [x] 密码重置对存在和不存在账户返回相同的公开响应。
* [x] 新分享密码使用 Argon2id，旧 SHA-256 分享仍可验证并升级。
* [x] 多连接并发访问不会突破分享访问/下载/流量限制。
* [x] API 上传与 Worker 清理并发时，不会删除仍有引用的对象。
* [x] 2 GiB 上限文件的扫描路径保持流式，内存占用不随文件大小线性增长。
* [x] 用户列表和后台列表有数据库分页，不再触发全库缓存刷新。
* [x] 慢登录、慢 S3 上传不会串行阻塞无关请求。
* [x] 请求取消能够终止相关数据库或对象存储操作。
* [x] JSON 超限及尾随内容被正确拒绝。
* [x] 生产环境会话 Cookie 始终带 `Secure`。
* [x] 核心文件按职责缩小，外部 HTTP API 保持兼容。
* [x] 合并后端语句覆盖率门禁达到至少 75%（含真实 PostgreSQL，覆盖全部项目 Go 包）。
* [x] 所有项目质量门禁通过。
