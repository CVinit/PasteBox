...
raw-secret evidence across the checklist and release notes. It also requires the
release notes' `Completed evidence checklist path` to match the checklist file
passed to `RELEASE_CHECKLIST`. Do not commit real secrets, raw provider
